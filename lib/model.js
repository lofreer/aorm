...
class Model {
    ...
    static update(_values, _options) {
        ...
        return Promise.try(() => {
            ...
            if (this.primaryKey && values[this.primaryKey]) {
    ...
    static updateMultiple(values, ids, _options = {}) {
        ...
        return Promise.try(() => {
            ...
            // 插值过滤
            values = values.map(value => {
                if (this.primaryKey && value[this.primaryKey]) {
    ...
    static normalizeAttribute(attributes) {
        return Utils.mapValues(attributes, (attribute, name) => {
            if (Utils.isString(attribute)) {
                ...
                if (!this.primaryKey && attribute.primaryKey) {
                    this.primaryKey = attribute.name
                }
            }
            return attribute
        })
    ...
    static whereBind(binding, key, value) {
        if (Utils.isObject(value)) {
            let keys = Object.getOwnPropertySymbols(value || {}).concat(Object.keys(value))
            value = keys.map(prop => {
                return this.whereItemQuery(key, {[prop]: value[prop]})
    ...
    static replaceAliases(obj) {
        ...
        let keys = Object.getOwnPropertySymbols(obj || {}).concat(Object.keys(obj))
        keys.map(key => {
            let value = obj[key]
            key = Op[key] || Op.Aliases[key] || key
