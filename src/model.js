...
class Model {
    ...
    static destroy(id) {
        return Promise.try(() => {
            return this.QueryInterface.destroy(this.tableName, id)
        })
    }

    static findById(id) {
        return Promise.try(() => {
            return this.QueryInterface.findById(this.tableName, id)
        }).then(res => {
            return res[0]
        })
    }
    ...
    static count() {
        return Promise.try(() => {
            return this.QueryInterface.count(this.tableName)
        }).then(result => {
    ...
    static normalizeAttribute(attributes) {
        attributes = Utils.mapKeys(attributes, (attribute, name) => {
            if (Utils.isString(attribute) || (Utils.isObject(attribute) && !attribute.name)) {
                return name
            }
    ...
    static whereItemQuery(key, value) {
        ...
        if (isObject) {
            if (OperatorMap[valueKeys[0]]) {
                return `${key} ${OperatorMap[valueKeys[0]]} ${value[valueKeys[0]]}`
            } else {
